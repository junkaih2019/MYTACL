...
class VGGLoss1(nn.Module):
    ...
    def __call__(self, x, y, z):
        if self.normalize is not None:
            ...
            y = self.normalize(y)
            z = self.normalize(z)
        x_vgg, y_vgg, z_vgg = self.vgg(x, self.indices), self.vgg(y, self.indices), self.vgg(z, self.indices)
        loss = 0
        for i in range(len(x_vgg)):
            loss += self.weights[i] * (self.criterion(x_vgg[i], y_vgg[i].detach()) / self.criterion(z_vgg[i], y_vgg[i].detach()))
